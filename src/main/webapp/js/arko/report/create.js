...
function init(){
	...
	report.type = RegExp.$1;
	console.log(report.type);
	setting = arko.getSetting(report.type);
	
	var element = $(".mws-panel-header span");
	element.empty();
	element.append($("<i>", {"class": "icon-magic"}));
	element.append("报告名称 - " + arko.getReportText(report.type));

	$("#preview-reportType span span").text(arko.getReportText(report.type));
...
//	}
	...
	$.getJSON(arko.getUrl("init.do"),null,function(json){
		report.id = json.id;
		report.number = json.number;
		$("#preview-reportId span span").text(report.number);
	});
...
//				preview();
	    ...
	    $.fn.pickList2 && $('.productgroup_pickList').pickList2({
			...
			preview : function(){
		        ...
		        $.each(report.filters, function(index, item){
		            var type = null, typeIndex = null, list = [];
		            
		            typeIndex = item.type;
		            
		            if(typeIndex!=2){
			            type = filter[typeIndex].text;
			            ...
			            $.each(item.items, function(index, tag){
			            	...
			            	var pid = tag.rel.split('-')[1];
			            	
			            	if(!p[pid]){
			            		var str="";
			            		$.each(filter[2], function(index, item){
			            			if(item.id==pid){
			            				str = item.name;
			            			}
			            		});
			            ...
			            $.each(p, function(index, item){
				            if(flag){
					            flag = 0;
				            	html += "满足" + item.name + "：" + item.list.join(" 或 ") + "<br/>";
				            }else{
					            html += "并且" + item.name + "：" + item.list.join(" 或 ") + "<br/>";
				            }
			            });
		            }
		        });
		...
		$.fn.pickList4 && $('.storegroup_pickList').pickList4({
	        ...
	        openDialog : openDialog,
          selectedData: [[],[{level:2,name:'线上店',type:8,value:409}]]
		});
	                    			...
	                    			if(flag){
	                    				if(!setting.productGroups.flag) {
		                    				...
		                    				if(total <= 0 ) {
	                							...
	                							msg = " 最多只允选择"+setting.productGroups.maxItem+"个商品!";
	                							flag = false;
		                    				}
	                    				}
	                    			}else{
	                    			 ...
	                    			 if(report.periods[0].start != null){
	                    				 if(report.periods[0].start>report.periods[0].end){
	                    					...
	                    					if(report.periods[0].start<startDate){
                    							...
                    							flag = false;
	                    					}/*else if((report.dateRange.datumPeriod.end.diff(report.dateRange.datumPeriod.start,'week')+1)>setting.periods.max){
		            							msg = " 基准时间段超出限制,最大只允许选择"+setting.periods.max+"周!";
			                    				flag = false;
			                    			}*/
		                    			}
	                    			}
	                    			if(flag && report.periods.length > 1 && report.periods[1].start != null){
	                    				if(report.periods[1].start>report.periods[1].end){
	                    					...
	                    					if(report.periods[1].start<startDate){
                    							...
                    							flag = false;
	                    					}/*else if((report.dateRange.comparePeriod.end.diff(report.dateRange.comparePeriod.start,'week')+1)>setting.periods.max){
		            							msg = " 比较时间段超出限制,最大只允许选择"+setting.periods.max+"周!";
			                    				flag = false;
			                    			}*/
	                    				}
	                    			}
                    ...
                    onSubmit: function() {
                    	...
                    	$.ajax({
                    		...
                    		success : function (json) {
                                ...
                                var buttons = {};
                    			if(json.status=="OK"){
                                    elements.append($("<i>",{'class':"icol-accept"})).append(" 您的报告已经递交成功，正在生成中，请稍候查看！");
                                    buttons = {
                    				    "继续创建报告": function () {
                    				        window.location.href = "/arko/report/select.do";
                    				    },
                    				    "查看详细": function () {
                    				        window.location.href = "/arko/report/"+report.id+"/view.do";
                    				    },
                    				    "OK": function () {
